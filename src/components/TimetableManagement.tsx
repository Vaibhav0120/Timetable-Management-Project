...
export const TimetableManagement: React.FC = () => {
  ...
  const [saveStatus, setSaveStatus] = useState<"Saved" | "Not Saved" | "Error">(
    "Saved"
  );
  ...
  const { timeSlots, addTimeSlot, updateTimeSlot, deleteTimeSlot } =
    useTimeSlots();
  const {
    timeTable,
    savedTimeTables,
  ...
  const handleTeacherChange = useCallback(
    (newTeacherId: number) => {
      if (selectedCell && selectedClass !== null && selectedSection !== null) {
        ...
        if (conflict) {
          const conflictingTeacher = teachers.find(
            (t) => t.id === newTeacherId
          );
          toast({
